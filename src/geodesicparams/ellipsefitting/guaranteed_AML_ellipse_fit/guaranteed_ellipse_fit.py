...
class EllipseData:
    ...
    def __init__(
        ...
        r,
        jacobian_matrix,
        jacobian_matrix_barrier,
        jacobian_matrix_full,
        hessian,
    ):
...
def guaranteedEllipseFit(t, data_points):
    ...
    elp_data = EllipseData(
        ...
        zeros(6, maxIter),
        zeros(6, maxIter),
        zeros(data_points.cols + 1, 1),
        zeros(data_points.cols, 6),
        zeros(1, 6),
        matrix(data_points.cols + 1, 6),
        0,
    )
    ...
    while keep_going and elp_data.k < maxIter:
        ...
        elp_data.jacobian_matrix_full[0 : elp_data.numberOfPoints, :] = (
            elp_data.jacobian_matrix
        )
        elp_data.jacobian_matrix_full[elp_data.numberOfPoints, :] = (
            elp_data.jacobian_matrix_barrier
        )

        # Approximate Hessian matrix
        elp_data.hessian = (
            elp_data.jacobian_matrix_full.T * elp_data.jacobian_matrix_full
        )
        ...
        if (
            elp_data.t[:, elp_data.k + 1].T * elp_data.f * elp_data.t[:, elp_data.k + 1]
        )[0, 0] <= 0:
        ...
        # Check for various stopping criteria to end the main loop
            ...
            min(
                norm(elp_data.t[:, elp_data.k + 1] - elp_data.t[:, elp_data.k]),
                norm(elp_data.t[:, elp_data.k + 1] + elp_data.t[:, elp_data.k]),
            )
            < elp_data.tolTheta
            and elp_data.theta_updated
        ):
            keep_going = False
        elif (
            fabs(elp_data.cost[elp_data.k] - elp_data.cost[elp_data.k + 1])
            < elp_data.tolCost
            and elp_data.theta_updated
        ):
            keep_going = False
        elif (
            norm(elp_data.delta[:, elp_data.k + 1]) < elp_data.tolDelta
            and elp_data.theta_updated
        ):
            keep_going = False
