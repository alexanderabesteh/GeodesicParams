...
References
----------
[] D. Foreman-Mackey, D. W. Hogg, D. Lang, and J. Good-man, emcee: The MCMC Hammer, 125, 306
...
from numpy import diff, percentile, vectorize
from numpy.random import randn
from scipy.optimize import minimize

from ..ellipsefitting import conv_coeffs_to_axis, fit_ellipse_3d
...
from .orbital_conversions import (
    convert_newtonian,
    convert_parametric_to_newton,
    orbital_elements,
)

# Constants
# In units of solar masses, astronomical units, years, and amperes
gLightSpeed = 63197.8021565
gGravConst = 39.4330827533
gPerm = 5.9608993448e22
gNut = 0
gMagnetic = 0
...
def fit_geodesic_orbit(
    data_points, init_theta, means, stdevs, sign_ang, init_dirs, steps, config
):
    """
    ...
    x_t1, y_t1, x_t2, y_t2 = convert_parametric_to_newton(
        coeffs, x1_close, y1_close, x2_close, y2_close, ecc, a
    )
    ...
    if len(init_theta) == 2:
        ...
        energy, ang_mom, carter = convert_newtonian(
            ecc, a, inc, sign_ang, b_rot, e_charge, gGravConst, gLightSpeed, gPerm
        )
    ...
    soln = minimize(
        nll, mod_theta, args=(x, y, z, ecc, a, inc, sign_ang, init_dirs, config)
    )
    ...
    sampler = EnsembleSampler(
        nwalkers,
        ndim,
        log_probability_emcee,
        args=(x, y, z, ecc, a, inc, sign_ang, init_dirs, means, stdevs, config),
    )
