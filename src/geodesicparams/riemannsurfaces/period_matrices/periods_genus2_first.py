...
from ..integrations.integrate_hyperelliptic import int_genus2_complex, myint_genus2
...
def ima4Per1(realNS, rea1, ima1, rea2, ima2, digits):
    ...
    r[0, 2] = (
        r[0, 3]
        + h1[0]
        - 1j * quad(lambda x: 1 / sqrt(-p(x)), [rea1, rea2])
        - re(h2[0])
        + 1j * im(h2[0])
    )
    r[1, 2] = (
        r[1, 3]
        + h1[1]
        - 1j * quad(lambda x: x / sqrt(-p(x)), [rea1, rea2])
        - re(h2[1])
        + 1j * im(h2[1])
    )
