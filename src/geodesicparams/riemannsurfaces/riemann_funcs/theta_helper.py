...
def sign_theta(n, z, tau):
    ...
    z_matrix = mp.matrix(
        [
            mp.mpc(f"{mp.re(z[0])}", f"{mp.im(z[0])}"),
            mp.mpc(f"{mp.re(z[1])}", f"{mp.im(z[1])}"),
        ]
    )
    t_matrix = mp.matrix(
        [
            [
                mp.mpc(f"{mp.re(tau[0, 0])}", f"{mp.im(tau[0, 0])}"),
                mp.mpc(f"{mp.re(tau[0, 1])}", f"{mp.im(tau[0, 1])}"),
            ],
            [
                mp.mpc(f"{mp.re(tau[1, 0])}", f"{mp.im(tau[1, 0])}"),
                mp.mpc(f"{mp.re(tau[1, 1])}", f"{mp.im(tau[1, 1])}"),
            ],
        ]
    )
...
def f(a, b, z, t):
    ...
    root_b = matrix(
        [mp.sqrt(b[i]) * sign_theta(i, mp.matrix([0, 0]), t) for i in range(n)]
    ).T
...
def to_inverse(a, b, z, t):
    ...
    rootB = [
        mp.sqrt(theta0withAequals0[i]) * sign_theta(i, mp.matrix([[0], [0]]), t)
        for i in range(n)
    ]
    ...
    jm1 = mp.matrix(
        [
            [-1 - 1 / tt[0, 0], -tt[0, 1] / tt[0, 0]],
            [-tt[0, 1] / tt[0, 0], tt[1, 1] - tt[0, 1] ** 2 / tt[0, 0]],
        ]
    )
    jm2 = mp.matrix(
        [
            [tt[0, 0] - tt[1, 0] ** 2 / tt[1, 1], -tt[0, 1] / tt[1, 1]],
    ...
    jm3 = mp.matrix(
        [
            [
                tt[0, 0] / detdetdet,
                (tt[0, 0] * tt[1, 1] - tt[0, 1] ** 2 - tt[0, 1]) / detdetdet,
            ],
            [
                (tt[0, 0] * tt[1, 1] - tt[0, 1] ** 2 - tt[0, 1]) / detdetdet,
                tt[1, 1] / detdetdet,
    ...
    lambda1, mu1 = agm_prime(
        mp.matrix(
            [
                [mp.mpf(1)],
                [sixteenThetas[9] / sixteenThetas[8]],
                [sixteenThetas[0] / sixteenThetas[8]],
                [sixteenThetas[1] / sixteenThetas[8]],
            ]
        ),
        mp.matrix(
            [
                [mp.mpf(1)],
                [sixteenThetaConstants[9] / sixteenThetaConstants[8]],
                [sixteenThetaConstants[0] / sixteenThetaConstants[8]],
                [sixteenThetaConstants[1] / sixteenThetaConstants[8]],
            ]
        ),
        zz1,
        jm1,
    ...
    lambda2, mu2 = agm_prime(
        mp.matrix(
            [
                [mp.mpf(1)],
                [sixteenThetas[0] / sixteenThetas[4]],
                [sixteenThetas[6] / sixteenThetas[4]],
                [sixteenThetas[2] / sixteenThetas[4]],
            ]
        ),
        mp.matrix(
            [
                [mp.mpf(1)],
                [sixteenThetaConstants[0] / sixteenThetaConstants[4]],
                [sixteenThetaConstants[6] / sixteenThetaConstants[4]],
                [sixteenThetaConstants[2] / sixteenThetaConstants[4]],
            ]
        ),
        zz2,
        jm2,
    ...
    lambda3, mu3 = agm_prime(
        mp.matrix(
            [
                [mp.mpf(1)],
                [sixteenThetas[8] / sixteenThetas[0]],
                [sixteenThetas[4] / sixteenThetas[0]],
                [sixteenThetas[12] / sixteenThetas[0]],
            ]
        ),
        mp.matrix(
            [
                [mp.mpf(1)],
                [sixteenThetaConstants[8] / sixteenThetaConstants[0]],
                [sixteenThetaConstants[4] / sixteenThetaConstants[0]],
                [sixteenThetaConstants[12] / sixteenThetaConstants[0]],
            ]
        ),
        zz3,
        jm3,
...
def diff_finies_one_step(a, b, z, t, lambda_iwant, det_iwant):
    ...
    to_inverse_base = to_inverse(
        mp.matrix(
            [[mp.mpc(1)], [my_a_of_size_3[0]], [my_a_of_size_3[1]], [my_a_of_size_3[2]]]
        ),
        mp.matrix(
            [[mp.mpc(1)], [my_b_of_size_3[0]], [my_b_of_size_3[1]], [my_b_of_size_3[2]]]
        ),
        z,
        t,
    )
