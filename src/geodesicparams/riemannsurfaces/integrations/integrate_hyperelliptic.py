...
def int_genus2_complex_exp(
    zeros, realPart, imaPart, position, exponent, branch, digits
):
    """
    ...
    iQ = lambda t: -coeffsg[3] * t**3 + t * (
        3 * coeffsg[3] * realPart**2 + 2 * coeffsg[2] * realPart + coeffsg[1]
    )
    rQ = (
        lambda t: -(t**2) * (3 * coeffsg[3] * realPart + coeffsg[2])
        + coeffsg[3] * realPart**3
        + coeffsg[2] * realPart**2
        + coeffsg[1] * realPart
        + coeffsg[0]
    )
    iQPrime = lambda t: -3 * coeffsg[3] * t**2 + (
        3 * coeffsg[3] * realPart**2 + 2 * coeffsg[2] * realPart + coeffsg[1]
    )
...
def eval_period(m, n, realNS, zeros, omega, component=0):
    ...
    periodlist = [
        omega[component, 0],
        omega[component, 2] - omega[component, 3],
        omega[component, 1],
        omega[component, 3],
        -omega[component, 0] - omega[component, 1],
    ]
    result = 0
...
def branch_list_genus2(zeros, num_realNS):
    ...
    if num_realNS == 5:
        ...
        # ima2Per3
            ...
            rea1 = re(zeros[2])
            if (
                (rea1 - zeros[0]) * (rea1 - zeros[1])
                + (rea1 - zeros[1]) * (rea1 - zeros[4])
                + (rea1 - zeros[4]) * (rea1 - zeros[0])
            ).evalf() < 0:
                erg = [
                    [1, zeros[0], 1],
                    [1, zeros[1], 0],
        ...
        if im(zeros[4]) == 0:
            ...
            if (
                (rea2 - rea1) ** 2 + ima1**2 + 2 * (rea2 - rea1) * (rea2 - zeros[4])
            ).evalf() < 0:
                erg = [
                    [1, zeros[0], 1],
                    [1, zeros[1], 0],
...
def int_genus2_first(zeros, lower, upper, digits, period_matrix):
    ...
    # ------------------ Case 2: only one of lb and ub is a real zero
        ...
        if inlist(lb, realNS) == -1:
            ...
            if inlist(lb, sorted([lb] + realNS, key=lambda x: re(x))) == inlist(
                ub, realNS
            ):
                return sign * myint_genus2(
                    e,
                    lb,
                    ub,
        ...
        # Case 2b: lb is a real zero
            ...
            # No real zeros between lb and ub
            if (
                inlist(ub, sorted([ub] + realNS, key=lambda x: re(x)))
                == inlist(lb, realNS) + 1
            ):
                return sign * myint_genus2(
                    e,
    ...
    # ---------------- Case 3: none of lb or ub is a real zero
        ...
        if inlist(lb, sorted([lb] + realNS, key=lambda x: re(x))) == inlist(
            ub, sorted([ub] + realNS, key=lambda x: re(x))
        ):
            if ub == oo:
                return sign * (
                    myint_genus2(
                    ...
                    + matrix(
                        [
                            eval_period(
                                len(realNS) - 1, oo, realNS, e, period_matrix, 0
                            ),
                            eval_period(
                                len(realNS) - 1, oo, realNS, e, period_matrix, 1
                            ),
                        ]
                    )
                )
                ...
                sign = sign * exp(
                    pi
                    * 1j
                    * branch_list_genus2(e, len(realNS))[
                        inlist(lb, sorted(e + [lb], key=lambda x: re(x)))
                    ][2]
                )
                return matrix(
                    [
                        sign * quad(lambda x: 1 / mpc(sqrt(p(x))), [lb, ub]),
                        sign * quad(lambda x: x / mpc(sqrt(p(x))), [lb, ub]),
                    ]
                )
        else:
            raise ValueError("Invalid bounds")
...
def int_second_genus2(zeros, pole, periods_first, periods_second, digits, minMax=5):
    ...
    u = int_genus2_first(zeros, zeros[1], pole, digits, periods_first) + matrix(
        riemann_const
    )
