...
from mpmath import chop, exp, fabs, im, ln, log, matrix, mpc, nint, quad, re, sign, sqrt
from numpy import array
from numpy import load as npload
from numpy import save
from sympy import Poly, Symbol, apart
from sympy import asin as spasin
from sympy import collect, degree, lambdify, nsimplify, oo, pprint
...
def integrate_eom(polynomial, zeros, substitution, integrand, datafile, digits):
    ...
    if deg_p <= 2:
        ...
        int_sym = int_sym[0]
        integrand_subs = lambdify(
            s, integrand.subs(int_sym, sol_nu).simplify(), "mpmath"
        )
    ...
    if deg_p == 5:
        ...
        def res_hyp(s):
            ...
            if len(rat_fracs_final) > 0:
                periodMatrix, invert_data, eps = npload(
                    f"{datafile}.npy", allow_pickle=True
                )
                ...
                rat_res = [
                    rat_fracs_final[0].as_numer_denom()[0] * integrations[0][i]
                    for i in range(len(integrations[0]))
                ]
                for j in range(1, len(integrations)):
                    rat_res = [
                        rat_res[i]
                        + rat_fracs_final[j].as_numer_denom()[0] * integrations[j][i]
                        for i in range(len(integrations[j]))
                    ]
                # pprint(integrations)
                # print()
                length = len(rat_res)
        ...
        def res_elp(s):
            ...
            integrations = [
                integrate_elliptic_third(i, datafile)(s) for i in rat_fracs_final
            ]
            ...
            for i in range(len(s)):
                ...
                for j in range(len(poly_second)):
                    zeta_bounds = weierstrass_zeta(
                        s[i] - inits[0], periods[0], periods[1]
                    )
...
def invert_elliptic_first(
    ...
    return lambda s: substitution(
        re(
            weierstrass_P(
                sqrt(constant) * s - initial_values[0] - int_initial,
                periodMatrix[0],
                periodMatrix[1],
            )
        )
...
def invert_hyperelliptic_first(
    ...
    if datafile != None:
        ...
        sol = lambda affine_list: orbitdata(
            initial_values,
            modified_init,
            [spsqrt(constant) * i for i in affine_list],
            substitution,
            initNewton,
            eps,
            max,
...
def integrate_hyperelliptic_first(component, datafile):
    ...
    result = [
        extended_orbitdata[i][3][component - 1] + invert_data[1][1][component - 1]
        for i in range(len(extended_orbitdata))
    ]
    return result
...
def orbitdata(
    ...
    if pos0 == -1:
        raise ValueError(
            f"Initial value {initial_values[0]} is not contained in list of affine parameters"
        )

    x = [initNewton]
    coordinates = [init_coord]
    subs_coord = [substitution(init_coord)]
    ...
    print(
        "Computing ",
        len(affine_list) - pos0,
        " solution points from i = ",
        pos0 + 1,
        " to ",
        len(affine_list),
        " ...",
    ...
    for i in range(pos0 + 1, len(affine_list)):
        coord = solution(
            affine_list[i] - modified_init, x[-1], eps, minMax, physical_comp
        )
        count = 0
        ...
        while len(coord) == 2 and re(coord[1]) < 1 and im(coord[1]) < 1 and count < 2:
            ...
            coord = solution(
                affine_list[i] - modified_init, coord[0][-1], eps, minMax, physical_comp
            )
            count += 1
        ...
        if len(coord) == 3 and im(coord[0]) < 100 * eps:
            ...
            print(
                f"WARNING in orbitdata: solution point for {affine_list[i]} ({i}. element) could not be computed result was {coord[0]}"
            )
            break
    ...
    for i in range(pos0 - 1, -1, -1):
        coord = solution(
            affine_list[i] - modified_init, x[0], eps, minMax, physical_comp
        )
        count = 0
        ...
        if len(coord) == 3 and im(coord[0]) < 100 * eps:
            ...
            print(
                f"WARNING in orbitdata: solution point for {affine_list[i]} ({i}. element) could not be computed."
            )
            break
    ...
    if datafile != None:
        extended_orbitdata = array(
            [
                [subs_coord[j], coordinates[j], x[j], divisor[j], affine_list[j]]
...
def sigma_ln_numerical(x, y, omega1, omega3):
    ...
    if im(value) != 0:
        value = chop(
            quad(
                lambda s: weierstrass_zeta(s - y, omega1, omega3)
                - weierstrass_zeta(s + y, omega1, omega3),
                [0, x],
            )
        )
...
def sigma_ln(x, y, omega1, omega3):
    ...
    for i in x:
        ...
        sigmatilde1 = weierstrass_sigma(i - y, omega1, omega3) * exp(
            -eta * (i - y) ** 2 / (2 * omega1)
        )
        sigmatilde2 = weierstrass_sigma(i + y, omega1, omega3) * exp(
            -eta * (i + y) ** 2 / (2 * omega1)
        )

        value = (
            log((sigmatilde1 / sigmatilde2) * exp(switch * (i / omega1 - 1)))
            - switch * (i / omega1 - 1)
            - 2 * eta * i * y / omega1
        )
        result.append(value)
...
def integrate_elliptic_third(integrand, datafile):
    ...
    def result(s):
        ...
        int_sol = (
            lambda s, y: 1
            / weierstrass_P(v1, periods[0], periods[1], 1)
            * (2 * (s - mod_int) * weierstrass_zeta(v1, periods[0], periods[1]) + y)
        )
...
def integrate_hyperelliptic_third(zeros, r1, r2, eta, integrand, datafile, digits):
    ...
    # in the remaining cases there is at least one real zero > pole!
        ...
        if im(zeros[1]) == 0 and im(zeros[0]) == 0:
            ...
            int_dz = matrix(
                [
                    1j * quad(lambda x: 1 / sqrt(-r(x)), [pole, re(zeros[1])]),
        ...
        if im(zeros[0]) == 0 and im(zeros[1]) == 0:
            ...
            int_dz = matrix(
                [
                    1j * quad(lambda x: 1 / sqrt(-r(x)), [pole, re(zeros[0])]),
            ...
            int_dz = matrix(
                [
                    1j * quad(lambda x: 1 / sqrt(-r(x)), [pole, zeros[0]]),
            ...
            +2 * myint_genus2_second(
                zeros, [0, 0, 1], zeros[0], re(zeros[1]), 1, digits
            )
            +2 * int_genus2_complex_second(
                zeros, [0, 0, 1], re(zeros[1]), fabs(im(zeros[1])), 1, 1, digits
            )

            int_3 = 2 * 1j * quad(lambda x: x**3 / sqrt(-r(x)), [pole, zeros[0]])
            +2 * myint_genus2_second(
                zeros, [0, 0, 0, 1], zeros[0], re(zeros[1]), 1, digits
            )
            +2 * int_genus2_complex_second(
                zeros, [0, 0, 0, 1], re(zeros[1]), fabs(im(zeros[1])), 1, 1, digits
            )
    ...
    def sigma(z):
        ...
        return hyp_theta_genus2(
            [1 / 2 * inverse_sum1, 1 / 2 * inverse_sum2], riemannM, char
        )
    ...
    def result(s):
        ...
        const = (
            (2 * matrix(divisor[pos0]).T * kappa * (xi_inf - yi_inf))
            + 1
            / 2
            * ln(
                sigma(
                    [divisor[pos0][0] + 2 * xi_inf[0], divisor[pos0][1] + 2 * xi_inf[1]]
                )
                / sigma(
                    [divisor[pos0][0] + 2 * yi_inf[0], divisor[pos0][1] + 2 * yi_inf[1]]
                )
            )
        )[0]
        vars = [
            sigma([divisor[i][0] + 2 * xi_inf[0], divisor[i][1] + 2 * xi_inf[1]])
            / sigma([divisor[i][0] + 2 * yi_inf[0], divisor[i][1] + 2 * yi_inf[1]])
            for i in range(len(divisor))
        ]
        log_vars = [
            (2 * matrix(divisor[i]).T * kappa * (xi_inf - yi_inf))[0]
            for i in range(len(divisor))
        ]
        ...
        res = [
            ...
            - const
            - (
                (divisor[pos0][0] + s0[0]) * int_dr1
                + (divisor[pos0][1] + s0[1]) * int_dr2
            )
        ]
        ...
        for i in range(pos0 + 1, len(divisor)):
            ...
            res.append(
                ...
                - const
                - (
                    (divisor[i][0] + s0[0]) * int_dr1
                    + (divisor[i][1] + s0[1]) * int_dr2
                )
            )
...
def solution_first(affineParameter, initNewton, eps, minMax):
    ...
    f = hyp_theta_genus2(z, riemannM, char)
    af = fabs(f)
    ...
    while af > eps and count < 30:
        a = (
            ...
            * (
                perR[0, 1]
                * hyp_theta_RR(
                    zfirstR, zfirstI, zsecondR, zsecondI, 0, riemannM, char, minMax
                )
                - perI[0, 1]
                * hyp_theta_IR(
                    zfirstR, zfirstI, zsecondR, zsecondI, 0, riemannM, char, minMax
                ...
                + perR[1, 1]
                * hyp_theta_RR(
                    zfirstR, zfirstI, zsecondR, zsecondI, 1, riemannM, char, minMax
                )
                - perI[1, 1]
                * hyp_theta_IR(
                    zfirstR, zfirstI, zsecondR, zsecondI, 1, riemannM, char, minMax
    ...
    s1 = hyp_theta_fourier(z, riemannM, char, [1], minMax)
    s2 = hyp_theta_fourier(z, riemannM, char, [2], minMax)
    sol = -(
        (s1 * periods_inverse[0, 0] + s2 * periods_inverse[1, 0])
        / (s1 * periods_inverse[0, 1] + s2 * periods_inverse[1, 1])
    )
