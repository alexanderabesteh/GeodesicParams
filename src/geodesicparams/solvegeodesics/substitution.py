...
def convert_deg5(polynomial, zeros):
    ...
    if coeff_5 < 0:
        ...
        substitution = y
        sign = 1
    return [p_standard, constant, lambdify(y, substitution, "jax"), substitution, sign]
...
def convert_degeven(polynomial, zeros, badzeros):
    ...
    return [
        data_degodd[0],
        prefactor,
        lambdify(Poly(data_degodd[3]).gen, substitution.subs(z, data_degodd[3]), "jax"),
        substitution.subs(z, data_degodd[3]),
        data_degodd[4],
    ]
...
def convert_polynomial(polynomial, degree, zeros, badzeros):
    ...
    if degree > 6:
        ...
        return [polynomial.subs(Poly(polynomial).gen, y), 1, lambdify(y, y, "jax"), y, -1]
